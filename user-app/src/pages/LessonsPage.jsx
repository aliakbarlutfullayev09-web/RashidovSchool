  ...
  useEffect(() => {
    async function loadData() {
      ...
      const [sData, cData, lData, pData] = await Promise.all([
        getSubjects(),
        getCourses(), // загружаем все курсы, будем фильтровать на клиенте
        getLessons(),
    ...
    <div className="h-full flex flex-col overflow-hidden bg-[#0f0c29]">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar pb-24 px-4 space-y-6 mt-4">
        ...
        {!selectedSubject ? (
          ...
          /* Список курсов выбранного предмета */
          <div className="space-y-6">
            ...
            <h2 className="text-2xl font-bold text-white px-1">{selectedSubject.name}</h2>
            ...
            {subjectCourses.map(course => {
              ...
              return (
                <div key={course.id} className="space-y-4">
                  <h3 className="text-xl font-bold text-white px-1">{course.title}</h3>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="relative w-full aspect-video rounded-2xl overflow-hidden bg-slate-800 shadow-lg border border-white/5 flex items-center justify-center">
                            ...
                            <span className="relative z-10 text-4xl opacity-30">
                              {selectedSubject.name.includes('Биолог') ? '🧬' : selectedSubject.name.includes('Хими') ? '🧪' : '📐'}
                            </span>
